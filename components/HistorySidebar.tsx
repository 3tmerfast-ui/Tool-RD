...
import { X, Clock, Trash2, ChevronRight, Image as ImageIcon, Loader2, Package, Shirt } from 'lucide-react';
              ...
              history.map((item) => (
                ...
                >
                  <div className="h-32 bg-slate-800 relative overflow-hidden">
                    ...
                    {/* Badge Tab - HIỂN THỊ LINH HOẠT THEO DỮ LIỆU */}
                    <div className={`absolute top-2 left-2 px-2 py-0.5 rounded text-[10px] font-bold flex items-center shadow-lg border ${
                      item.tab === AppTab.TSHIRT 
                      ? 'bg-purple-600 text-white border-purple-400' 
                      : 'bg-indigo-600 text-white border-indigo-400'
                    }`}>
                       {item.tab === AppTab.TSHIRT ? (
                         <><Shirt size={10} className="mr-1" /> T-SHIRT</>
                       ) : (
                         <><Package size={10} className="mr-1" /> POD</>
                       )}
                    </div>
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-[10px] text-slate-600 uppercase font-bold tracking-widest">{item.designMode === 'NEW_CONCEPT' ? 'New Concept' : 'Enhance'}</span>
                      <button 
